...
public class SubCommandRemove implements ConfigSubCommandExecutor {
    ...
    public void execute(Server server, User user, ServerVoiceChannel voiceChannel, TextChannel textChannel, Map<String, ConfigSubCommandExecutor> subs, String[] args, MessageCreateEvent event) {
        if (event.getMessage().getMentionedUsers().size() != 1) {
            textChannel.sendMessage(Main.getErrorEmbed(I18n.format(server.getId(), "command.config.add.error.no_mention"), server));
            return;
        }
        ...
        textChannel.sendMessage(Main.getSuccessEmbed(I18n.format(server.getId(), "command.config.remove.successful.title"), String.format(I18n.format(server.getId(), "command.config.remove.successful.description"), mentionnedUser.getMentionTag())));
