...
public class SubCommandAddRole implements ConfigSubCommandExecutor {
    @Override
    public void execute(Server server, User user, ServerVoiceChannel voiceChannel, TextChannel textChannel, Map<String, ConfigSubCommandExecutor> subs, String[] args, MessageCreateEvent event) {
        ...
        if (args.length != 1) {
            event.getChannel().sendMessage(EmbedUtils.getErrorEmbed(I18n.format(event.getMessage().getServer().get().getId(), "command.config.add-role.error.no_role_precised"), event.getMessage().getServer().get()));
            return;
        }
        ...
        new MatchRoleQuery(textChannel, roleMatches).then((role) -> {
            ...
            voiceChannel.createUpdater()
                    .addPermissionOverwrite(
                            ...
                            Permissions.fromBitmask(References.USER_ALLOWED_PERMISSION_SHOWN,
                                    References.USER_DENIED_PERMISSION_SHOWN)
                    )
                    .update();
            ...
            textChannel.sendMessage(
                    EmbedUtils.getSuccessEmbed(
                            I18n.format(
                                    server.getId(),
                                    "command.config.add-role.successful.title"),
                            String.format(I18n.format(server.getId(),
                                    "command.config.add-role.successful.description"),
                                    r.get().getName()
                            )
                    )
            );
    ...
    public String getDescription(Server server) {
        return I18n.format(server.getId(), "command.config.add-role.description");
    }
