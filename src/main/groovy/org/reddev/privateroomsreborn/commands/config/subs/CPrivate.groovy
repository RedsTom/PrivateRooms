...
class CPrivate implements
    ...
    void execute(MessageCreateEvent event, BotConfig config, String cmd, String[] args) {
        ...
        channel.private_ = true
        channel.hidden = false
        channel.update(event.api).thenAccept {
            event.channel.sendMessage(l("cmd.config.private.success", event.server.get()))
        }
    }

    @Override
    CommandDescriptor getDescriptor(Server guild) {
        new CommandDescriptor(description: l("cmd.config.private.description", guild))
    }
}
